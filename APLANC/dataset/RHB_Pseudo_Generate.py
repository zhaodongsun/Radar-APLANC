...
# 项目根目录是当前文件的父目录的父目录 (nndl/rf → nndl)
...
def Pseudo_Label_Generator(cpkt_path,datapath, datapaths,save_root, window_size=5, samples=256, samp_f=5e6, freq_slope=60.012e12):
        ...
        rf_file_list = datapaths  # [1_1,2_5,........]
        ...
        for rf_file in rf_file_list:
            ...
            data_p = np.array([np.real(raw_data_p),np.imag(raw_data_p)])# (2,T,D)
...
def main():
    datapath = 'APLANC/dataset/RHB'
    folds_path = 'APLANC/dataset/RHB_demo_fold1.pkl'
    save_root = 'APLANC/dataset/Pseudo_Labels_RHB'
    ...
    cpkt_path = 'APLANC/rf/ckpt/RHB/best.pth'
